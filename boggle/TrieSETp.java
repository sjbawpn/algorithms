 ...
 *  @author Kevin Wayne
 */
public class TrieSETp implements Iterable<String> {
    private static final int R = 26;        // extended ASCII

    private Node root;      // root of trie
    private int N;          // number of keys in trie
    private HashMap<String, Boolean> prefixList = new HashMap<String, Boolean>();
    private HashMap<String, Boolean> containsList = new HashMap<String, Boolean>();
    ...
    public boolean contains(String key) {
        ...
        if (containsList.containsKey(key)) return containsList.get(key);
        Node n = root;
        int d = 0;
        String s = null;

        if (key.length() > 2) {
            s = key.substring(0, key.length() - 2);
        }
        if (s != null && nodeList.containsKey(s)) {
            n = nodeList.get(s);
            d = s.length();
        }
        Node x = get(n, key, d);
        if (x != null && x.isString) result = true;
        containsList.put(key, result);
        return result;
    }
    ...
    private Node get(Node x, String key, int d) {
        ...
        if (nodeList.containsKey(key)) return nodeList.get(key);
        if (d == key.length()) return x;
        char c = key.charAt(d);
        n = get(x.next[c-65], key, d+1);
        nodeList.put(key,n);
        return n;
    }
    ...
    public boolean hasMore(String prefix) {
        boolean result = false;

        if (prefixList.containsKey(prefix)) return prefixList.get(prefix);
        Node n = root;
        int d = 0;
        String s = null;

        if (prefix.length() > 2) {
            s = prefix.substring(0, prefix.length() - 2);
        }
        if (s != null && nodeList.containsKey(s)) {
            n = nodeList.get(s);
            d = s.length();
